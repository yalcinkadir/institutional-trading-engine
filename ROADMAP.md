...
| PO7 | Add deterministic Daily Observation Record Index for review, status counts, duplicate-date rejection and paper-only audit discovery | P0 | Critical | Done / CI-green |
...
| PO12 | Add deterministic Daily Observation Artifact Retention & Review Index for PO10/PO11 artifacts with status counts, review readiness and paper-only audit metadata | P0 | Critical | Done / CI-green |
| PO13 | Add deterministic Monthly Paper Observation Review Pack with REVIEW_READY/BLOCKED status from the PO12 review index | P0 | Critical | Done / CI-green |
...
PO7 adds the deterministic index `reports/daily_observation_records/index.json` for PO3/PO5 records. The index stores total records, ACCEPTED / REJECTED / NEEDS_REVIEW counts, canonical record paths, review flags, evidence/incident counts and the paper-only safety boundary. PO7 does not authorize live trading.

PO8 builds the deterministic Daily Observation Review Summary from the PO7 index. It reports total records, accepted/rejected/needs-review counts, review-required dates, rejected dates, needs-review dates and `review_ready` while preserving `live_trading_authorized=false` and `broker_execution_mode=paper_only`.
...
PO12 builds the deterministic Daily Observation Artifact Retention & Review Index at `reports/daily_observation_automation/review_index.json`. It summarizes PO10/PO11 automation artifacts, retention metadata, passed/blocked counts, review-ready counts, gate status, blocker/error counts and the paper-only safety boundary. PO12 does not authorize live trading.

PO13 builds a deterministic monthly Paper Observation Review Pack at `reports/monthly_paper_observation_review/YYYY-MM.json`. It converts the PO12 review index into monthly `REVIEW_READY` or `BLOCKED` status, including passed/blocked days, review-ready days, gate failures, blocker/error counts and rejected/needs-review record counts. PO13 does not authorize live trading.
