...
Hard rule: no real-money execution before real forward evidence, drift detection, regime-change monitoring, position-level risk attribution and manual approval are in place.
...
| BT6 | Add previous-run evidence baseline comparison for regression gates | P1 | High | Implemented / CI workflow added |
| BT7 | Add capacity and turnover realism gates before any private production sizing work | P1 | High | Planned |

BT5 is a robustness gate only. BT6 is a baseline-regression gate only. Neither proves production edge and neither authorizes live trading.
...
C3, C4, C5, C6 and C7 are implemented as public paper-execution/evidence infrastructure only. They do not add proprietary strategy edge and do not authorize live trading.
...
- BT6 Evidence Baseline Regression Gate: implemented with tests, CLI, demo fixture, documentation and dedicated CI workflow.
- BT5 Walk-Forward / Out-of-Sample Robustness Gate: done and CI-green.
- BT5 dedicated workflow: done and CI-green.
- BT5 CLI, synthetic demo fixture and operational documentation: done.
...
B1.1 remains the long-running evidence collection period. Phase C is active for paper execution only. Telegram delivery is allowed for research/paper-observation reports only. BT6 is implemented with a dedicated CI workflow. Immediate focus remains controlled evidence collection, safe reporting, public/private edge separation and the next backtest evidence-hardening gate after BT6 is confirmed green.
