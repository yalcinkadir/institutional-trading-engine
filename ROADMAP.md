...
| RGP4 | Escalate Polygon/data-provider fetch failure for actionable signals into explicit degraded/fail evidence instead of silently skipping lifecycle evaluation | P0 | Critical | Done / CI-wired |
| RGP5 | Send or persist STOP/EXIT alerts before git commit/rebase/push so alert delivery cannot be blocked by repository persistence failures | P0 | Critical | Implemented / CI-wired |
