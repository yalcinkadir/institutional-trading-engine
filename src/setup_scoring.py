...
def _volume_series(bars: list[dict]) -> list[float]:
    return [float(bar.get("v", 0)) for bar in bars]
...
def calculate_asymmetry_score(bars: list[dict]) -> float:
    ...
    downside_to_sma50 = max(close - sma50, atr14)
...
def score_setup(symbol: str, asset_bars: list[dict], benchmark_bars: list[dict]) -> SetupScore:
    ...
    trend_quality = calculate_trend_quality(asset_bars) if len(asset_bars) >= 200 else 0.3
    ...
    rv = relative_volume(volumes[-1], avg_volume_20)
    rv_score = _clamp(rv / 1.5)

    asymmetry = calculate_asymmetry_score(asset_bars) if len(asset_bars) >= 200 else 0.4
