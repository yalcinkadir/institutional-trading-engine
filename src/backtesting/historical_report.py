...
def render_markdown(report: HistoricalBacktestReport) -> str:
    ...
    lines = [
        ...
        f"- Pipeline coupled: {report.pipeline_coupled}",
        f"- Pipeline generation source: {report.pipeline_generation_source}",
        f"- Generated signals: {report.generated_signal_count}",
        f"- Validated trade plans: {report.validated_trade_plan_count}",
        f"- Blocked signals: {report.blocked_signal_count}",
        f"- Runtime gates applied: {', '.join(report.runtime_gates_applied) if report.runtime_gates_applied else 'none'}",
