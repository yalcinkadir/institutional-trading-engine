    ...
    for delay in (1, 2, 3):
        specs.append(
            VariantSpec(
                ...
                description=(
                    "Proxy for next-bar close confirmation using BT136 MFE/MAE path summary."
                ),
                parameters={
    ...
    for threshold in (1.1, 1.25, 1.5):
        specs.append(
            VariantSpec(
                ...
                description=(
                    "Volume-confirmed breakout requiring entry volume versus average volume."
                ),
...
def _accepted_by_variant(row: dict[str, Any], spec: VariantSpec) -> bool:
    ...
    if spec.family == "next_bar_close_confirmation":
        delay = int(spec.parameters["confirmation_delay_bars"])
    ...
    if spec.family == "volatility_adjusted_confirmation":
        ...
        return (
            mae >= float(spec.parameters["max_adverse_excursion_r_allowed"])
            and mfe >= float(spec.parameters["minimum_mfe_r"])
        )
...
def _metrics_for_period(
    ...
    if status != STATUS_EVALUATED:
        return PeriodMetrics(
            ...
            accepted_trades=0,
            skipped_trades=len(period_rows),
...
def _recommend(
    ...
    if train_delta > 0 and oos_delta < 0:
        return RECOMMEND_OVERFIT_RISK, (
            "In-sample improves while out-of-sample degrades."
        )
...
def analyze(evidence_path: Path) -> BT133Report:
    ...
    for spec in specs:
        ...
        if spec.family in {
            "next_bar_close_confirmation",
            "volatility_adjusted_confirmation",
        }:
            limitations.append(
                "BT131/BT136 evidence has MAE/MFE summaries, not exact intrabar "
                "path ordering; result is research proxy only."
            )
    ...
    final = _final_recommendation(variant_results)
        ...
        lines.extend(
            _markdown_table(
                ...
                [
                    [
                        period.period,
                        period.total_trades_considered,
                        period.accepted_trades,
                        period.skipped_trades,
                        period.false_breakout_rate,
                        period.stop_hit_rate,
                        period.target_1_hit_rate,
                        period.target_2_hit_rate,
                        period.average_r,
                        period.expectancy_r,
                    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze BT131/BT136 evidence into BT133 entry-confirmation variant reports"
        )
    )
...
def main() -> int:
    ...
    print(
        json.dumps(
            {
                "status": report.final_recommendation,
                "variants": len(report.variant_results),
            },
            indent=2,
        )
    )
