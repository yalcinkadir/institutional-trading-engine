...
#192: Scheduled report liveness gate writes reports/scheduled_report_liveness/<date>-<type>-liveness.json, requires non-empty scheduled report/latest artifacts, requires signals and paper-health evidence for market reports, and blocks missing scheduled output from being counted as a productive report cycle. CI run 27472903940 passed.
...
#186: Outcome tracking differentiates SUCCESS, BLOCKED_NO_VALID_SIGNALS, DEMO_NO_DATA and BLOCKED_MISSING_INPUTS; production outcome learning is not claimable when upstream signal files are empty, invalid or contain zero valid signals. CI run 27473216252 passed.
