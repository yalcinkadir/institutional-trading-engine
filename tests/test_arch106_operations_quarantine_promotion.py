...
from scripts.promote_arch106_operations_quarantine_candidates import OPERATIONS_QUARANTINE_CANDIDATES, promote_candidates


def _write_base_classification(path: Path, *, baseline_limit: int = 2) -> None:
...
def test_arch106_operations_quarantine_promotion_is_isolated_not_runtime(tmp_path: Path) -> None:
    repo = tmp_path
    for candidate in OPERATIONS_QUARANTINE_CANDIDATES:
    ...
    assert classification["classified_modules"] == OPERATIONS_QUARANTINE_CANDIDATES
