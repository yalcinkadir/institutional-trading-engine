...
from scripts.promote_arch106_os_delete_candidates import OS_DELETE_CANDIDATES, promote_candidates


def _write_base_classification(path: Path, *, baseline_limit: int = 3) -> None:
...
def test_arch106_os_delete_candidate_promotion_keeps_modules_present(tmp_path: Path) -> None:
    repo = tmp_path
    for candidate in OS_DELETE_CANDIDATES:
    ...
    assert classification["classified_modules"] == OS_DELETE_CANDIDATES

    inventory = build_inventory(repo_root=repo, classification_path=classification_path)
    assert inventory["counters"]["classified_modules"] == 3
    assert inventory["counters"]["delete_candidate"] == 3
