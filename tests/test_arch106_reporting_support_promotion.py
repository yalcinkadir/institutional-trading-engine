...
from scripts.promote_arch106_reporting_support_modules import REPORTING_SUPPORT_CANDIDATES, promote_candidates


def _write_base_classification(path: Path, *, baseline_limit: int = 4) -> None:
...
def test_arch106_reporting_support_promotion_is_experimental_without_runtime_proof(tmp_path: Path) -> None:
    repo = tmp_path
    for candidate in REPORTING_SUPPORT_CANDIDATES:
    ...
    assert classification["classified_modules"] == REPORTING_SUPPORT_CANDIDATES

    inventory = build_inventory(repo_root=repo, classification_path=classification_path)
    assert inventory["counters"]["classified_modules"] == 4
    assert inventory["counters"]["experimental"] == 4
