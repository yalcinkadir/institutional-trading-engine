...
def test_positive_regime_setup_entry_profile_increases_score(tmp_path: Path):
...
def test_strong_positive_profile_increases_more_selectively(tmp_path: Path):
    history = _write_history(
        tmp_path / "history.json",
...
def test_negative_profile_reduces_score_and_size(tmp_path: Path):
    history = _write_history(
        tmp_path / "history.json",
